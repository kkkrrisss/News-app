//
//  NewsListViewMode.swift
//  NewsApp
//
//  Created by Кристина Олейник on 17.08.2025.
//
...
class NewsListViewModel: NewsListViewModelProtocol {
    ...
    private var isSearchTextChange: Bool = false
    //MARK: - Methods
    
    func loadData(searchText: String? = nil) {
        if self.searchText != searchText {
            page = 1
            isSearchTextChange = true
        } else {
            page += 1
    ...
    func convertCellViewModel(_ articles: [ArticleResponseObject]) {
        let viewModels = articles.map { ArticleCellViewModel(article: $0) }
        
        if sections.isEmpty || isSearchTextChange {
